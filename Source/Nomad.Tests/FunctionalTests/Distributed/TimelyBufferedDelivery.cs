...
namespace Nomad.Tests.FunctionalTests.Distributed
{
    ...
    public class TimelyBufferedDelivery : DistributedNomadBase
    {
        ...
        [Test]
        public void
            local_module_publishes_5_outdated_and_5_valid_messages_and_later_loaded_listener_module_receives_those_messages
            ()
        {
            // path for this test (using the test method name) use in each code
            PrepareSharedLibrary();
        ...
        private void AssertEventPublished(string listener)
        {
        ...
        [Test]
        public void distributed_module_publishes_and_later_loaded_listener_module_receives_those_binaryBuffered_messages()
        {
            // path for this test (using the test method name) use in each code
            PrepareSharedLibrary();
            ...
            // postponed load of the listener module should trigger deserialization and delivery of those messages to the localEA
            IModuleDiscovery listenerDiscovery = new SingleModuleDiscovery(listener1);
            ListenerKernel.LoadModules(listenerDiscovery);
